  ...
  render () {
    ...
    return <div>
      ...
      <div className="docContent">
        <div className="row">
          <div className="eight columns">
            <em>Note: This is an unofficial mirror of the docs. For the real thing, please see the <a href="https://plot.ly/javascript/reference/" target="_blank">plotly.js figure reference</a>.</em>
          </div>
        </div>
        {Object.keys(this.state.schema.traces).map(type => {
          if (this.state.schema.traces[type].attributes._visible) cnt++;
          return <Trace key={type} type={type} schema={this.state.schema.traces[type]}/>
        })}
        {cnt === 0 ? (
          <div className="row">
